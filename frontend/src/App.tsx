...
import { HostKeyDialog } from "./features/connections/HostKeyDialog";
import {
  findTabByPtySessionId,
  hostKeyTrustLabel,
  isInteractiveSshReady,
} from "./features/connections/connection-state";
import type { ConnectionSubmitIntent } from "./features/connections/connection-form";
import { ErrorNotice } from "./features/errors/ErrorNotice";
import { AppShell } from "./features/shell/AppShell";
import { useSshEvents } from "./features/ssh/useSshEvents";
import { base64ToBytes } from "./features/terminal/base64";
import {
  PtyOutputBuffer,
...
import { submitConnectionProfile } from "./features/workspace/connection-submit";
...
function App() {
  ...
  const [operationFailure, setOperationFailure] =
    useState<OperationFailure | null>(null);
  ...
  const sshEventListenerState = useSshEvents(
    (event) => {
      if (event.event === "ssh.connection.status") {
        ...
        try {
          const bytes = outputBuffer.ingest({
          ...
          setTabs((current) => {
            const target = findTabByPtySessionId(
              current,
  ...
  const disconnectSession = async (sshSessionId: string) => {
        ...
        explorer={
          <ConnectionExplorer
            ...
            onDisconnect={(sshSessionId) =>
              void disconnectSession(sshSessionId)
            }
          />
        }
        terminal={
          <TerminalWorkspace
            ...
            errorNotice={operationFailure ? (
              <ErrorNotice
                error={operationFailure.error}
                partialSuccess={operationFailure.profileSavedBeforeConnect}
              />
            ) : null}
            onSelectConnection={() => {
