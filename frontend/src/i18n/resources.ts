const en = {
  ...
  activity: { approval: "Approval", settings: "Settings", filesUnavailable: "Files are planned for M3" },
  connections: {
  ...
  sftp: {
    ...
    recoveryActions: { deleteTemp: "Delete temporary file", continueDelete: "Continue delete", restoreTombstone: "Restore tombstone", openLocalFolder: "Open local folder", keep: "Keep for later" },
  ...
  agent: {
    ...
    message: "Message", messagePlaceholder: "Ask Agent to inspect or operate on this SSH session…", provider: "Provider", chooseProvider: "Choose Provider", providerSettings: "Provider settings", openProviderSettings: "Open Provider settings", send: "Send message", enterToSend: "Enter to send · Shift+Enter for newline", running: "Waiting for complete result…", thinking: "Thinking…", newConversation: "New conversation", resetTitle: "Start a new conversation?", resetBody: "This clears only the current tab's in-memory conversation view.", confirmReset: "Start new conversation", riskTitle: "Allow experimental remote command execution?", riskBody: "Agent can execute shell commands in this SSH Session. Commands may modify the remote host; there is no per-command approval, reliable frontend stop, or production safety guarantee.", confirmRisk: "Acknowledge and send", compactRisk: "Experimental Agent: commands may modify the remote host and cannot be reliably stopped from this UI.", emptySession: "Select a connected terminal tab to use Agent.", emptyProvider: "Configure and select an enabled Provider before sending.", runDetails: "Run details", sentSnapshot: "Sent snapshot", runId: "Run ID", runStatus: "Status", iteration: "Iteration", session: "Session", apiType: "API type", model: "Model", noMessages: "No messages in this tab.", completedAnnouncement: "Agent completed for {{name}}", failedAnnouncement: "Agent failed for {{name}}", tabRunning: "Agent running for {{name}}", tabCompleted: "Agent completed for {{name}}", tabFailed: "Agent failed for {{name}}", activeRunTitle: "Agent Run is still active", activeRunBody: "{{name}} still owns an active Agent Run.",
    errors: { UI_AGENT_ACTIVE_SESSION_REQUIRED: "A connected SSH Session is required.", UI_AGENT_PROVIDER_REQUIRED: "Select an enabled Provider.", UI_AGENT_PROVIDER_UNAVAILABLE: "The selected Provider is no longer available.", UI_AGENT_MESSAGE_INVALID: "Message length must be between 1 and 65536 characters.", UI_AGENT_FINAL_TEXT_MISSING: "The completed Run did not return final text.", UI_MODEL_API_CONFIG_ACTIVE_RUN: "The Provider is in use by an active Run." },
  },
  settings: { language: "Language", general: "General", close: "Close settings", diagnostics: { title: "Diagnostics", description: "Harness Shell writes local application and Agent logs here.", loading: "Resolving log directory…", path: "Log directory", open: "Open log directory", opening: "Opening…" }, modelProviders: { title: "Model Providers", newProvider: "New Provider", empty: "No model providers configured.", loading: "Loading providers…", retry: "Retry", enabled: "Enabled", disabled: "Disabled", storedKey: "API Key safely stored", activeRun: "In use by an active Run", createTitle: "New Provider", editTitle: "Edit Provider", deleteTitle: "Delete Provider?", deleteBody: "Delete {{name}} and its stored API Key?", confirmDelete: "Confirm delete", displayName: "Display name", apiType: "API type", baseUrl: "Base URL", model: "Model", apiKey: "API Key", keepCurrentKey: "Leave blank to keep the current API Key", required: "Required", invalid: "Invalid value", save: "Save", partialSuccess: "The Provider changed, but cleanup failed.", primaryFailure: "Provider operation failed", cleanupFailure: "Vault cleanup failed" } },
...
const zhCN = {
  ...
  activity: { approval: "审批", settings: "设置", filesUnavailable: "文件功能计划在 M3 提供" },
...
const zhTW = {
  ...
  activity: { approval: "審批", settings: "設定", filesUnavailable: "檔案功能預計於 M3 提供" },
