...
User → React WebView → typed API wrapper → Tauri command
     ...
     → Python Sidecar Router / Handler → SSH、PTY 或 Storage
...
| Python Sidecar | Protocol 请求处理、SSH/PTY 生命周期、契约授权的用户手动 SFTP remote I/O/mutation、Runtime SQLite、Audit、Trace | WebView UI、DPAPI Vault、未经契约授权的远程写操作 |
...
- Connection profile 的持久化权威是 Sidecar schema v3 的单调 `version`：创建为 `1`，每次成功更新原子 `+1`，上限为 `2^53-1`；Rust 在解析凭据后把目标与 ProxyJump 的快照作为数字 `profile_version` 发送，Sidecar 在任何网络 I/O 前校验，`updated_at` 只用于展示。
- Sidecar 崩溃、heartbeat 超时或协议终止错误必须显式进入失败或暂停状态，不得静默重启并重放请求。
- 旧的 Sidecar 内部 Agent exec/SFTP/Artifact 运行时已删除；用户手动 SFTP 的私有 Rust coordinator/wire、Python remote I/O/mutation、仅限固定 `main` window 的 21 个 typed Tauri commands/单一 `sftp` permission，以及非持久化 typed Frontend API/controller/workspace 已完成实现。SFTP Activity 在进入时只绑定当时显式选中的 connected terminal tab，并保持该绑定直到离开后重新进入；切换普通 active tab 不迁移 listing 或 transfer owner。进入时隐藏 Agent pane 但保留其偏好；统一自动门禁与 Tauri Desktop 人工验收仍是独立待确认层。该能力不得成为 Agent、M3 Workflow、approval window 或 WebView raw Protocol 能力。
