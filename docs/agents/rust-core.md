...
- DPAPI Vault、Sidecar 进程、RuntimeClient、Supervisor、Job Object 或 RuntimeStatus；
- Rust HTTP/WebSocket model、WebView 事件或 packaged backend 测试。
...
- 通过专用模型 API Key kind 与 Agent Tauri commands 注入最短生命周期的 secret request；
...
- HTTP/WebSocket runtime client、packaged child/Job 与 lifecycle：[frontend/src-tauri/src/runtime/](../../frontend/src-tauri/src/runtime/)
...
- `src/commands/`：按领域组织 Tauri commands；负责 WebView 边界校验、Vault 取值和构造 sealed typed runtime requests。非 SFTP commands 直接使用 `RuntimeClientHandle`；Manual SFTP commands 只进入独立 coordinator，由 coordinator 使用同一 handle。
- `src/runtime/`：strict HTTP models/client、typed WebSocket actor、dynamic-port process owner、Windows Job、无 global transport sequence 的 public RuntimeStatus 与 production lifecycle supervisor。Tauri setup 先 manage pending `RuntimeClientHandle`，只有 live → initialize → ready → WebSocket heartbeat 全部成功后才发布；失败或关闭时先 revoke。
- `src/sftp/`：用户手动 SFTP 的 Rust coordinator、sealed typed HTTP/binary runtime client、跨进程严格模型、本地文件 owner 与 DPAPI-protected journal；coordinator 通过注入的 typed sink 发布 transfer projection，production sink 由 `lib.rs` 固定绑定 `main` window 与 `manual-sftp://transfer-state`。
...
- `src/commands/agent.rs`：四个 Provider metadata CRUD commands 与一个 turn command；turn 先通过 typed HTTP 读取当前 config，再从 Vault 解析专用 API Key kind，并只通过脱敏且 drop-zeroize 的 typed Agent turn request 传给 Sidecar。`commands/credentials.rs` 另提供两个模型 API Key store/delete commands，因此 Agent 后端新增七个 main-window Tauri commands。
...
- `tests/`：跨模块契约测试，覆盖 capability、HTTP/WebSocket Runtime、Supervisor、Vault、Manual SFTP 和 packaged backend。
...
- Typed HTTP request 使用唯一 `X-Request-ID` 并严格关联 response；未知、重复或类型错误的 response 必须 fail closed。
- Typed HTTP client 不设置统一 total request deadline；Runtime startup 由 Supervisor 的 5 秒 deadline 约束，SSH command、Manual SFTP、Provider retry/cancel 等业务耗时由各领域 owner 约束。
...
- Sidecar live、一次性 initialize、ready 与首个 WebSocket ping/pong 握手全部成功后才能发布 READY。
...
- `RuntimeRequest` 与 HTTP initialize body 的 `Debug` 必须持续脱敏 payload/runtime keys；initialize model 持有的 key strings 在 drop 时清零。
...
- 模型 API Key 只存在于 Rust DPAPI Vault；Sidecar metadata 仅保存 opaque secret reference。Rust 解析后使用 canonical Base64 放入 sealed Agent HTTP request，不得进入 `Debug`、event、普通 error 或持久化明文。
...
- Production Runtime WebSocket actor 是 WebView runtime event 白名单真源，只对 `ssh://event` 与 `manual-sftp://operation-state` 做 typed projection。新增 event 必须同时显式设计 Python schema、Rust model/projection、Frontend validator 和测试。
- Rust-local manual SFTP transfer event 不经过 Runtime WebSocket：production 必须显式注入 main-window sink，sink 只接受 `TransferProgressProjection`。event emit 失败记录 path-free stable diagnostic，但不能使已 dispatch transfer 失败或触发 replay。
...
- Runtime HTTP/WebSocket 固定使用 Rust 选择的 `127.0.0.1:<dynamic-port>`；WebView 不得获得 base URL。当前不实现 auth、TLS 或 remote access，且不允许 HTTP fallback、retry middleware、WebSocket reconnect 或 child respawn。
...
需要验证 packaged backend 契约时，先构建当前二进制并显式设置路径：
...
- HTTP/WebSocket：运行 `runtime_models_contract.rs`、`runtime_http_contract.rs`、`runtime_websocket_contract.rs`，覆盖 request 关联、Problem Details、无统一 total request deadline、未知事件、invalid message、heartbeat 与 pending PTY reply 清理。
- Vault：运行 `vault_contract.rs`，覆盖 DPAPI 密文、runtime key、删除和 secret non-exposure。
- Supervisor：运行 `runtime_supervisor_contract.rs` 和 `packaged_backend_http_contract.rs`，覆盖 dynamic port、ready、heartbeat、shutdown、异常退出和 Job 资源清理。
...
- RuntimeClient、Supervisor、RuntimeStatus、heartbeat、timeout 或进程策略改变；
- Rust module 或契约测试布局改变；
- packaged backend 构建路径、环境变量或 Rust 测试命令改变。
