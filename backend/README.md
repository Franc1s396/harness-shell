...
```powershell
.\.venv\Scripts\python.exe -m harness_ssh_sidecar serve --port 8765 --data-dir E:\absolute\harness-ssh-dev
```

安装版只能由 `harness-ssh-launcher.exe` 启动：

```text
harness-ssh-sidecar.exe desktop --port 0 --data-dir <absolute> --control-read-handle <n> --ready-write-handle <n>
```
