...
def build_auth_options(
    *,
    ...
    if auth_kind == "password":
        if password is None:
            raise _auth_error("SSH_PASSWORD_REQUIRED")
        try:
            password_text = bytes(password).decode("utf-8")
        except UnicodeDecodeError as exc:
            raise _auth_error("SSH_PASSWORD_ENCODING_INVALID") from exc
        return {
    ...
    try:
        ...
        if encrypted is True and passphrase is not None:
            raise _auth_error("PRIVATE_KEY_PASSPHRASE_INVALID") from exc
        raise _auth_error("PRIVATE_KEY_UNSUPPORTED") from exc
...
def _auth_error(error_code: str) -> SshRuntimeError:
