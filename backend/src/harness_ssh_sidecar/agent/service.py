...
LOGGER = logging.getLogger("harness_ssh_sidecar.agent.service")
...
class AgentService:
    ...
    async def delete_conversation(self, conversation_id: UUID) -> None:
        ...
        from harness_ssh_sidecar.storage.orm import AgentRunRow
        with self._database.read_session() as session:
    ...
    async def _invoke_until_terminal(self, initial_state: AgentGraphState, context: AgentGraphContext) -> AgentGraphState:
        ...
        saver = InMemorySaver(serde=JsonPlusSerializer(pickle_fallback=False, allowed_msgpack_modules=[
            ("harness_ssh_sidecar.agent.context_models", "ContextMessage"),
            ("harness_ssh_sidecar.agent.context_models", "ContextSummary"),
            ("harness_ssh_sidecar.agent.contracts", "AgentRunStatus"),
        ]))
