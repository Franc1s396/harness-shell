...
- stdout 必须保持为空；所有日志写 stderr，格式固定为 `timestamp | level | reqId | thread | logger | message`，timestamp 使用设备本地系统时间，调用点只提交经过安全审查的稳定字段。源码 `serve` 模式为 PyCharm 等开发控制台输出 ANSI 分级颜色；`desktop` 模式保持无 ANSI 的纯文本，由 Launcher 原样写入独立 Backend 轮转日志。
- `telemetry/logging.py` 独占 console formatter 与 request ID context。业务代码直接调用标准 `Logger.debug/info/warning/error/exception` 并使用 `%s` 参数化，不再增加日志 helper wrapper；异常日志遵循 [Python Style Guide](../../../docs/agents/python-style.md#异常与失败传播)。Logger 不替调用方做秘密扫描，route、model 和 I/O 调用点不得主动在日志参数中加入 request/response body、credential 或远程输出。
...
- Agent 已知领域异常的 Problem/SSE message 必须原样使用异常产生点明确审查的 `safe_message`，不得再按 error code 二次映射；未知异常继续使用固定安全文本，禁止暴露任意异常文本或异常链。
